...
export const CATALOG: Record<string, ItemDef> = Object.fromEntries(
  [
    ...
    def('obsidianshard', 'Obsidian', 'Minerals', 'uncommon', 'obsidian', 44, 52, { c: ['#2a2a30', '#0a0a10'] }),
    def('meteoriron', 'Meteorite Iron', 'Minerals', 'epic', 'meteor', 48, 46, { c: ['#3a3a3a', '#18181a'] }),
    def('coin', 'Ancient Coin', 'Artifacts', 'common', 'coin', 32, 32, { c: ['#d4af6a', '#7a5c28'] }),
    // The `opts.c` colors added below (potshard through idol) aren't read by
    // their flat-icon draw routines in sprites.ts (shard/amphora/axe/mask/
    // ring/amulet/pearl/idol all hard-code their own palette already) — they
    // exist solely so the 3D museum inspect view (src/data/models.ts,
    // getTintColor) can tint each item's real-world-sourced model toward
    // its actual expected color instead of falling back to rarity color.
    def('potshard', 'Pottery Shard', 'Artifacts', 'common', 'shard', 44, 42, { c: ['#b5773f', '#7a4a24'] }),
    def('amphora', 'Amphora', 'Artifacts', 'rare', 'amphora', 52, 74, { c: ['#b5773f', '#7a4a24'] }),
    def('axe', 'Stone Axe Head', 'Artifacts', 'uncommon', 'axe', 50, 44, { c: ['#8a8578', '#5c584e'] }),
    def('mask', 'Ceremonial Mask', 'Artifacts', 'epic', 'mask', 54, 66, { c: ['#c8a44a', '#8a6a20'] }),
    def('goldring', 'Gold Ring', 'Treasure', 'rare', 'ring', 36, 40, { c: ['#e8c860', '#a87c20'] }),
    def('amulet', 'Jade Amulet', 'Treasure', 'epic', 'amulet', 40, 52, { c: ['#3aa8a0', '#1c5a54'] }),
    def('pearl', 'Pearl', 'Treasure', 'rare', 'pearl', 30, 30, { c: ['#ffffff', '#b0a0a8'] }),
    def('idol', 'Golden Idol', 'Treasure', 'mythic', 'idol', 48, 66, { c: ['#f5d878', '#a87c20'] }),
    def('geode', 'Giant Geode', 'Gems', 'epic', 'geode', 110, 110, null, true),
    def('chest', 'Sunken Treasure Chest', 'Treasure', 'legendary', 'chest', 150, 120, { c: ['#8a5a2c', '#5a3818'] }, true),
